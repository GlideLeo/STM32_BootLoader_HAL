              ...
              else
              {
                if (packets_received == 0)//文件信息(首包)
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)//文件名字
                  {
                    ...
                    file_name[i++] = '\0';//字符串形式
										OLED_printf(0xFFFF," name %s\n",file_name);
                    for (i = 0, file_ptr ++; (*file_ptr != ' ') && (i < FILE_SIZE_LENGTH);)
                    {
                      file_size[i++] = *file_ptr++;
                    }
                    file_size[i++] = '\0';
										OLED_printf(0xFFFF," size %s\n",file_size);
...
//                    FLASH_ProgramWord(FlashDestination, *(uint32_t*)RamSource);
										...
										if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,FlashDestination,*(uint32_t*)ramsource) == HAL_OK)
										{
											
											//OLED_printf(0xFFFF," %d",*(uint32_t*)ramsource);
										}
										OLED_printf(0xFFFF," %d",*(uint32_t*)FlashDestination);
